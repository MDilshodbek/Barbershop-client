...
import { CREATE_SERVICE, UPDATE_SERVICE } from "../../../apollo/admin/mutation";
...
const AddService = ({ initialValues, ...props }: any) => {
  ...
  const [insertServiceData, setInsertServiceData] =
    useState<ServiceInput>(initialValues);
  ...
  const {
    ...
    variables: { input: router.query.serviceId },
  });
  ...
  useEffect(() => {
    setInsertServiceData({
      ...insertServiceData,
      serviceTitle: getServiceData?.getService
        ? getServiceData?.getService?.serviceTitle
        : "",
      servicePrice: getServiceData?.getService
        ? getServiceData?.getService?.servicePrice
        : 0,
      serviceType: getServiceData?.getService
        ? getServiceData?.getService?.serviceType
        : "",
      serviceDuration: getServiceData?.getService
        ? getServiceData?.getService?.serviceDuration
        : 0,
      serviceDesc: getServiceData?.getService
        ? getServiceData?.getService?.serviceDesc
        : "",
      serviceImages: getServiceData?.getService
        ? getServiceData?.getService?.serviceImages
        : [],
    });
  ...
  const doDisabledCheck = () => {
    if (
      ...
      insertServiceData.serviceType === "" || // @ts-ignore
      insertServiceData.serviceDesc === "" || // @ts-ignore
      insertServiceData.serviceDesc === "" ||
                ...
                <Stack className="price-year-after-price">
                  ...
                  <input
                    ...
                    onChange={({ target: { value } }) =>
                      setInsertServiceData({
                        ...insertServiceData,
                        servicePrice: value === "" ? 0 : parseInt(value, 10),
                      })
                    }
                  />
                </Stack>
              ...
              <Stack className="config-row">
                <Stack className="price-year-after-price">
                  ...
                  <input
                    ...
                    onChange={({ target: { value } }) =>
                      setInsertServiceData({
                        ...insertServiceData,
                        serviceDuration: value === "" ? 0 : parseInt(value, 10),
                      })
                    }
                  />
                </Stack>
                <Stack className="price-year-after-price">
                  ...
                  <select
                    ...
                    value={insertServiceData.serviceType || "select"}
                    onChange={({ target: { value } }) =>
                      // @ts-ignore
                      setInsertServiceData({
                        ...insertServiceData,
